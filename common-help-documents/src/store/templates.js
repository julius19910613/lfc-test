...
// 获取一个条件组的模板
function getConditionGroupTemplate(type, defaultSlotNames) {
  const slotNames = defaultSlotNames || [''];
  const criteriaSet = {
    operator: 'OR',
    items: [
      {
        operator: 'AND',
        items: slotNames.map(v => ({
          left: v,
...
// 根据用户所选解析器，自动生成结束的模板，所有解析器槽位都有值时结束
function generateTerminationsBySlots(slotNames) {
  const newCond = {
    criteriaSet: getConditionGroupTemplate('exit', slotNames),
    nlTemplates: {
      options: [''],
    },
  };
