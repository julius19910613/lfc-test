  ...
  <div class="dropdown-select" :style="styleObj">
    ...
    <div ref="list" v-if="show" class="select-list" :style="listStyle">
      <div class="button-list">
        <el-button size="mini" class="select-button" @click="setAll(false)">{{ $t('general.clear_all') }}</el-button>
        <el-button size="mini" class="select-button" @click="setAll(true)">{{ $t('general.pink_all') }}</el-button>
      </div>
...
export default {
  ...
  methods: {
    ...
    emitUpdate() {
      const ret = [];
      this.selfOptions.map((category) => {
        ret.push({
          key: String(category.key),
          value: category.values.filter(x => x.checked).map(x => x.key),
        });
        return ret;
      });
      this.$emit('input', ret);
