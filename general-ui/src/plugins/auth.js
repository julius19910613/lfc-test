...
const ENTERPRISE_PATHV5 = '/auth/v5/enterprise';
...
function getRobots(userInfo) {
  ...
  // enterprise admin
  if (userInfo.type === 1) {
    return that.$reqGet(`${ENTERPRISE_PATHV5}/${userInfo.enterprise}/apps`)
      .then(rsp => rsp.data.result);
  }
  // system admin
  if (userInfo.type === 0) {
    const currentEnterprise = window.localStorage.getItem('enterprise');
    if (currentEnterprise === '' || currentEnterprise === undefined || !currentEnterprise) {
      return new Promise(r => r([]));
    }

    return that.$reqGet(`${ENTERPRISE_PATHV5}/${currentEnterprise}/apps`)
      .then(rsp => rsp.data.result);
  }
...
function setInfoWithToken(token) {
  ...
  .then(() => {
    if (userInfo.type !== 0) {
      ...
      .then(() => that.$reqGet(`${ENTERPRISE_PATHV4}/${enterprise}/modules`))
      .then((rsp) => {
        const data = rsp.data;
        modules = data.result.filter(mod => mod.is_show);
      });
    }
...
function clearAuth() {
  ...
  localStorage.removeItem('robots');
  localStorage.removeItem('topMenuBar');
