  ...
  methods: {
    ...
    goEnterprise(enterprise) {
      ...
      that.$api.getEnterpriseModules(enterprise.enterpriseID).then((modules) => {
        const showModules = modules.filter(mod => mod.is_show);
    ...
    popEditEnterprise(enterprise) {
      ...
      const promises = [
        ...
        that.$api.getModules(),
        that.$api.getEnterpriseModulesV3(enterprise.enterpriseID),
        that.$api.getProducts(),
        that.$api.getEnterpriseProductsV5(enterprise.enterpriseID),
      ];

      return Promise.all(promises).then((datas) => {
        const [enterprises, modules, enterpriseModules, products, enterpriseProducts] = datas;
        that.$pop({
          ...
          extData: {
            ...
            checkedProducts: enterpriseProducts,
          },
          left_button: {
    ...
    popAddEnterprise(initData) {
      ...
      return Promise.all(promises).then((datas) => {
        const [enterprises, modules, products] = datas;
        that.$pop({
    ...
    addEnterprise(data) {
      ...
      .catch((err) => {
        ...
        } else if (err.response.data.ret_msg === 'Column input error: name') {
          that.$notifyFail(that.$t('management.err_enterprise_duplicate'));
        } else {
