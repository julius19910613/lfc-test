<template>
  <div class="form">
    ...
    <div class="row module-title">
      <div class="row-title part" ref="moduleTitle">{{ $t('management.product_list') }}</div>
    </div>
    <div class="row multi-line">
      <div class="module" v-for="(prod, idx) in products" :key="idx">
        <input :id="`module-${idx}`" type="checkbox" v-model="prod.checked">
        <label :for="`module-${idx}`" class="module-name">{{ prod.name }}</label>
    ...
    <div class="row multi-line">
      <div v-for="(mod, idx) in modules" v-show="mod.code !== 'ssm' && mod.code !== 'qa_label'" :key="idx" class="module">
        ...
        <label :for="`module-${idx}`" class="module-name">{{ mod.name }}</label>
