...
const ENTERPRISE_V4_URL = '/auth/v4/enterprise';
...
const ENTERPRISE_URL_V5 = '/auth/v5/enterprise';

function getEnterprises() {
  return this.$reqGet(ENTERPRISES_URL).then(rsp => rsp.data.result);
}

function getEnterpriseModulesV3(id) {
  return this.$reqGet(`${ENTERPRISE_URL}/${id}/modules`).then(rsp => rsp.data.result);
}

function getEnterpriseProductsV5(id) {
  return this.$reqGet(`${ENTERPRISE_URL_V5}/${id}/products`).then(rsp => rsp.data.result);
}

function getEnterpriseModules(id) {
  return this.$reqGet(`${ENTERPRISE_V4_URL}/${id}/modules`).then(rsp => rsp.data.result);
}
...
function addEnterprise(info, enableIM) {
  ...
  data.products = JSON.stringify(info.products);
...
export default {
  ...
  updateEnterprise,
  deleteEnterprise,
  getEnterpriseModulesV3,
  getEnterpriseProductsV5,
};
