    ...
    <div class="card h-fill w-fill">
      ...
      <div id="audit-result">
        <div id="audit-toolbar">
          ...
          <div v-if="showTable" id="audit-log-count">{{ $t('management.audit.total', { num: totalLogCount }) }}</div>
        </div>
  ...
  computed: {
    pageOption() {
      if (this.userType === UserType.SYSTEM_ADMIN) {
        ...
        this.currentPage = 'auditEnterprise';
        return {
          auditEnterprise: this.$t('management.audit.enterprise'),
          auditRobot: this.$t('management.audit.robot'),
        };
      }
      this.currentPage = 'auditRobot';
      return {
        auditRobot: this.$t('management.audit.robot'),
      };
    },
