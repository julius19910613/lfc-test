<template>
  <div class="user-info-form" :class="[isAdmin ? 'admin' : 'normal']">
    ...
    <div class="row" v-for="(privilege, idx) in privilegeSet" :key="idx">
      ...
      <dropdown-select class="selector" :options="machineOptions" v-model="privilege.machine" :width="'100%'" :placeholder="$t('general.please_choose')"/>
      <div class="row-text">{{ $t('management.privilege_is') }}</div>
      <dropdown-select class="selector" :options="privilegeOptions" v-model="privilege.role" :width="'100%'" :placeholder="$t('general.please_choose')"/>
