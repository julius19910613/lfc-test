          ...
          ok: (scenarioName) => {
            taskEngineApi.createScenario(that.appId, scenarioName).then((data) => {
              if ('template' in data && 'metadata' in data.template) {
                ...
                // general.popErrorWindow(that, that.i18n.task_engine_v3.
                // error_msg.create_new_scenario_failed, '');
                this.$message({
                  message: that.i18n.task_engine_v3.error_msg.create_new_scenario_failed,
                  type: 'error',
                  showClose: true,
                });
              }
              ...
              this.$message({
                message: `createScenario error: ${err.message}`,
                type: 'error',
                showClose: true,
              });
            });
          ...
          ok() {
            taskEngineApi.deleteScenario(scenario.scenarioID).then((data) => {
              if ('msg' in data && data.msg === 'Update success') {
                ...
                // general.popErrorWindow(that,
                // 'deleteScenario error', 'failed to delete scenario.');
                this.$message({
