  ...
  methods: {
    ...
    saveScenario(content) {
      return api.saveScenario(
        ...
        this.$message({
          message: `saveScenario error: ${err.message}`,
          type: 'error',
          showClose: true,
        });
      });
    ...
    publishScenario(content) {
      ...
      this.saveScenario(convertedContent).then(() => {
        api.publishScenario(this.appId, this.scenarioId).then(() => {
          ...
          this.$message({
            message: `publishScenario error: ${err.message}`,
            type: 'error',
            showClose: true,
          });
        });
    ...
    enableScenario() {
      api.switchScenario(this.appId, this.scenarioId, true).then(() => {
        ...
        this.$message({
          message: `enableScenario error: ${err.message}`,
          type: 'error',
          showClose: true,
        });
      });
    ...
    isValidScenario() {
      ...
      if (this.scenarioName === '') {
        ...
        this.$message({
          message: this.i18n.task_engine_v3.error_msg.please_enter_the_scenario_name,
          type: 'error',
          showClose: true,
        });
        return false;
