...
<div id="condition-block">
  ...
  <div class="normal-edge" v-if="edgeType==='normal' || edgeType==='global_normal'">
    <template v-for="(rule, index) in andRules">
      <div class="rule-block" :key="rule.id">
        ...
        <div class="content-parser" v-if="rule.funcName === 'custom_parser'">
          <div class="row">
            <div class="label label-start smallfont">
            ...
            <dropdown-select
              ...
              @hide="hidetooltip('dropdownSelect')"
            /><span ref="spanDropSelect" class='forValid'>{{rule.content.parser}}</span>
          </div>
          <div class="row">
            <span class="label label-start" v-t="'task_engine_v2.condition_block.label_custom_parser_slot'"></span>
            <dropdown-select
              class="select"
              :value="[rule.content.slot.slotId]"
              @input="customParserSlotInput(index, $event[0])"
              :options="rule.content.parserData.slotInfo.map(slot => ({text: slot.slotName, value: slot.slotId}))"
              :showCheckedIcon="false"
              :showSearchBar="true"
              :clearcheckflag="true"
              width="420px"
    ...
    <div class="row" v-if="tab !== 'TriggerEditTab'">
      <div class="label label-start">
    ...
    <div class="row" v-if="tab !== 'TriggerEditTab'">
      <span class="label label-start" v-t="'task_engine_v2.condition_block.label_tag_enable'"></span>
      <toggle v-model="toNodeLabelsEnable" :size="'medium'" :showLabel="true" :label="toggleLabel"></toggle>
    </div>
    <div class="row " v-if="tab !== 'TriggerEditTab' && toNodeLabelsEnable">
      <span class="label label-start" v-t="'task_engine_v2.condition_block.label_tag'"></span>
      <div class="dropdown-container">
        <tag-input ref='tagref'
    ...
    <div class="row2 row-no-bottom-margin" v-if="tab !== 'TriggerEditTab' && toNodeLabelsEnable">
      <div style="background:#fff;width:420px;border:1px solid #dedede; margin-left:84px;">
...
export default {
  ...
  methods: {
    addlitext(idx) {
      this.$refs.tagref.addTagBySelector(idx);
    },
    hidetooltip(ref) {
      const inputContentRefs = this.$refs[ref];
      if (inputContentRefs) {
    ...
    renderNormalEdge() {
      ...
      this.origToNodeLabels = this.edge.to_node_labels || [];
      this.toNodeLabels = this.edge.to_node_labels || [];
      this.toNodeLabelsEnable = this.edge.to_node_labels_enable || false;
    },
    deleteEdge() {
    ...
    isValid() {
      this.valdflag = 0;
      const inputContents = this.$refs.spanDropSelect;
      if (inputContents) {
    ...
    showToolTip() {
      ...
      const inputContentRefs = this.$refs.dropdownSelect;
      if (inputContentRefs) {
        let inputContents = inputContentRefs;
        if (!Array.isArray(inputContents)) {
          inputContents = [inputContents];
    ...
    customParserSlotInput(index, slotId) {
      if (slotId === '-1') {
        this.andRules[index].content.slot = {};
      } else {
        this.andRules[index].content.slot = this.andRules[index].content.parserData.slotInfo.filter(
  ...
  mounted() {
    console.log(this.$refs['input-content']);
