      ...
      <div class="parser-block" :key="parser.id">
        ...
        <div class="content-parser" v-if="parser.funcName === 'custom_parser'">
          ...
          <div class="row">
            ...
            <dropdown-select
              class="select row-content"
              :value="[parser.content.slot.slotId]"
              @input="customParserSlotInput(index, $event)"
              :options="parser.content.parserData.slotInfo.map(slot => ({text: slot.slotName, value: slot.slotId}))"
              :showCheckedIcon="false"
              :showSearchBar="true"
              :clearcheckflag="true"
              width="420px"
...
export default {
  ...
  methods: {
    ...
    dropDownUpdate() {
      this.dropRefs = [];
      this.parsers.forEach((item, index) => {
    ...
    isValid() {
      ...
      for (let i = 0; i < this.dropRefs.length; i += 1) {
        ...
        this.$nextTick(() => {
          if (el.length > 0) {
            if (el[0].checkedValues.length === 0) {
              this.validfalg = 1;
            }
          }
        });
    ...
    showToolTip() {
      ...
      const selectRefs = this.dropRefs;
      if (selectRefs.length > 0) {
        selectRefs.forEach((item) => {
          item.forEach((el) => {
            if (el.value[0] === '') {
    ...
    customParserSlotInput(index, newValue) {
      const slotId = newValue[0];
      if (slotId === '-1') {
        this.parsers[index].content.slot = {};
      } else {
        this.parsers[index].content.slot = this.parsers[index].content.parserData.slotInfo.filter(
