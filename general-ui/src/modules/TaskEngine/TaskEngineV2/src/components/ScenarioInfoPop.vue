...
<div id="scenario-info">
  ...
  <div class="row">
    <div class="title" v-t="'task_engine_v2.scenario_info.scenario_timeout'"></div>
    <input ref="nodetimeoutCnt" type="number"  v-model.trim="timeoutLimit" v-tooltip="nodetimooutTooltip" @focus="inputFocus"/>
  </div>
  <div class="row">
...
export default {
  ...
  data() {
    return {
      ...
      updateTime: general.convertIsoString2LocalDateTimeString(this.value.metadata.update_time),
      ...
      nodetimooutTooltip: {
        msg: this.$t('task_engine_v2.scenario_info.timeoutTooltip'),
        eventOnly: true,
        errorType: true,
        alignLeft: true,
      },
      timeoutLimit: this.value.setting.timeoutLimit || 100,
    };
  },
  methods: {
    validate() {
      ...
      if (!this.$refs.nodetimeoutCnt.value || this.timeoutLimit < 30 || this.timeoutLimit > 3600) {
        valid = false;
        this.$refs.nodetimeoutCnt.dispatchEvent(event.createEvent('tooltip-show'));
      }
