...
import voiceSetConvertor from './voiceSetConvertor';

export default {
  convertUiNodesToNodes(uiNodes, setting, globalEdges, voiceSet, globalSilenceSwitch) {
    const nodes = uiNodes.map((uiNode) => {
      const newUiNode = JSON.parse(JSON.stringify(uiNode));
      return this.convertUiNodeToNode(newUiNode, setting, globalEdges, voiceSet,
      globalSilenceSwitch);
    });
  ...
  convertUiNodeToNode(uiNode, setting, globalEdges, voiceSet, globalSilenceSwitch) {
    // console.log(uiNode);
    ...
    if (uiNode.nodeType === 'entry') {
      ...
      // 语音设置的环境变量为ture,才加上边。
      if (voiceSet) {
        node.tts_info = this.convertUiNodeTottsInfo(uiNode);
        if (!nodeInfo.is_last_node) {
          node.edges = this.addEdgesTTSINfo(uiNode, globalEdges, globalSilenceSwitch);
        }
      }
    } else if (uiNode.nodeType === 'sub_scenario') {
      node.content = this.composeDialogue2Content(uiNode);
      node.scenarios = uiNode.subScenarioTab.normalEdges;
    }
    return node;
  },
  getNodeInfo(edges) {
    let isLastNode = true;
    edges.forEach((edge) => {
      if (edge.edge_type === 'global_normal' || edge.edge_type === 'global_normal_2.0') return;
  ...
  convertUiNodeTottsInfo(uiNode) {
    const info = uiNode.ttsInfo;
    let bargeinValue = '';
    let silenceDuration = '';
    let vadValue = '';
    let vspValue = '';
    if (info && (info.bargein || info.silence || info.vad || info.vsp)) {
      bargeinValue = info.bargein.bargein_value;
      silenceDuration = info.silence.silence_duration;
      vadValue = info.vad.vad_value;
      vspValue = info.vsp.vsp_value;
    }
    const ttsInfo = {
      bargein: {
        bargein_value: bargeinValue,
      },
      silence: {
  ...
  convertUiNodeToEdges(uiNode, setting, initialGlobalEdges) {
    ...
    if (uiNode.nodeType === 'entry') {
      ...
      const elseInto = this.edgeElseIntoWithLabel(uiNode.nodeId, uiNode.edgeTab);
      edges = [
        ...hiddenEdges,
        ...normalEdges,
        ...globalEdges,
      ...
      const normalEdges = this.addResetDialogueCntAndParseFailedAction(uiNode.edgeTab.normalEdges);
      const exceedThenGoto = this.edgeExceedThenGoToWithLabel(uiNode.edgeTab);
      const elseInto = this.edgeElseIntoWithLabel(uiNode.nodeId, uiNode.edgeTab);
      edges = [
        hiddenSetCntLimit,
        ...hiddenEdges,
    ...
    } else if (uiNode.nodeType === 'dialogue_2.0' || uiNode.nodeType === 'sub_scenario') {
      const hiddenSetCntLimit = this.edgeHiddenSetNodeDialogueCntLimit(
        uiNode.edgeTab2.dialogueLimit,
      );
      const normalEdges = uiNode.edgeTab2.normalEdges;
      const useCommonEdge = uiNode.edgeTab2.useCommonEdge;
      const exceedThenGoto = this.edgeExceedThenGoToWithLabel(uiNode.edgeTab2);
      const elseInto = this.edgeElseIntoWithLabel(uiNode.nodeId, uiNode.edgeTab2);
      // 旧task 兼容
      let actEdge = {};
      // 如果一个act没选也不添加edge
      if (uiNode.understandTab && uiNode.understandTab.acttableData.length > 0) {
        actEdge = this.edgeActTab(uiNode);
      }
      if (useCommonEdge) { // 是否添加globalEdge
        edges = [
          hiddenSetCntLimit,
          ...normalEdges,
          ...globalEdges,
      ...
      if (Object.keys(actEdge).length !== 0) {
        edges.splice(1, 0, actEdge);
      }
    }
    return edges;
  },
  addEdgesTTSINfo(uiNode, initialGlobalEdges, globalSilenceSwitch) {
    let globalEdges = [];
    if (uiNode.edgeTab2.useCommonEdge) {
      globalEdges = this.changeSuffixOfGlobalEdges(
        uiNode.nodeId,
        this.addResetDialogueCntAndParseFailedAction(initialGlobalEdges),
      );
    }
    ...
    const exceedThenGoto = this.edgeExceedThenGoToWithLabel(uiNode.edgeTab2);
    const elseInto = this.edgeElseIntoWithLabel(uiNode.nodeId, uiNode.edgeTab2);
    const ttsInfo = uiNode.ttsInfo;
    let bargeinFlag;
    const keyIncrease = [];
    const edgeRemoveKey = [];
    const exitSpeech = [];
    const silenceSpeech = [];
    let silenceSwitch = globalSilenceSwitch;
    if (ttsInfo) {
      bargeinFlag = uiNode.ttsInfo.bargein.bargein_value;
      keyIncrease[0] = voiceSetConvertor.edgekeyIncrease();
      edgeRemoveKey[0] = voiceSetConvertor.edgeRemoveKey();
      exitSpeech[0] = voiceSetConvertor.exitSpeech(ttsInfo);
      silenceSpeech[0] = voiceSetConvertor.silentSpeech(ttsInfo);
      silenceSwitch = ttsInfo.silence.silence_switch;
    }
    // 旧task 兼容
    let actEdge = {};
      // 如果一个act没选也不添加edge
    ...
    const bargein = [];
    if (bargeinFlag) {
      bargeinParser[0] = voiceSetConvertor.bargeinParser();
      bargein[0] = voiceSetConvertor.bargeinEdge(ttsInfo);
    }
    if (silenceSwitch) { // 全局静默话术配置true
      edges = [
        hiddenSetCntLimit,
        ...keyIncrease,
        ...edgeRemoveKey,
  ...
  edgeActTab(uinode) {
    const tab = uinode.understandTab;
    const id = uinode.nodeId;
    const actions = [];
    const tableData = tab.acttableData;
    const typeValue = tab.actTypeValue;
    tableData.forEach((el, index) => {
      actions.push({
        source: 'text',
        function: {
          function_name: 'dialog_act_parser',
          content: {
            parserData: {
              classifierId: el.classifierId,
              classifierName: el.classifierName,
              description: el.description,
              predictSlotName: el.predictSlotName,
              predictSlotDesc: el.predictSlotDesc,
        ...
        type: 'parser',
        parser: 'dialog_act_parser',
        dialogActParserType: typeValue[index],
      });
    });
    const edge = {
  ...
  addResetDialogueCntAndParseFailedAction(initialEdges) {
    ...
    return edges.map((edge) => {
      if (edge.edge_type !== 'normal_2.0' && edge.edge_type !== 'global_normal_2.0') {
    ...
    return globalEdges.map((edge) => {
      if (edge.condition_rules &&
        ...
        if (edge.edge_type === 'global_normal') {
          ...
          edge.condition_rules = edge.condition_rules.map((rule) => {
            if (rule.function) {
    ...
    edges.forEach((edge) => {
      if (edge.edge_type === 'normal_2.0') {
        ...
        if (andRules instanceof Array) {
          ...
          let func;
          if (andRules.function) {
            func = andRules.function;
          }
          const vars = this.getGlobalVarsFromFunction(func);
          globalVars.push(...vars);
        }
  ...
  getGlobalVarsFromFunction(func) {
    ...
    if (funcName === 'regular_exp') {
    ...
    } else if (funcName === 'self_increment') {
      if (func.content && Array.isArray(func.content)) {
        vars = [func.content[0].key];
      }
      ...
      vars = [func.content];
    } else if (funcName === 'custom_parser') {
      if (func.skipIfKeyExist) {
        vars = func.skipIfKeyExist;
      }
    }
  ...
  edgeExceedThenGoToWithLabel(edgeTab) {
    return {
      to_node_id: edgeTab.exceedThenGoto,
      to_node_labels: edgeTab.exceedThenGotoLabels,
      to_node_labels_enable: edgeTab.exceedThenGotoLabelsEnable,
      edge_type: 'exceedThenGoTo',
      edge_name: '超出轮回次数',
      condition_rules: [
        [
  ...
  edgeElseIntoWithLabel(nodeId, edgeTab) {
    const edge = {
      to_node_id: edgeTab.elseInto,
      to_node_labels: edgeTab.elseIntoNodeLabels,
      to_node_labels_enable: edgeTab.elseIntoNodeLabelsEnable,
      edge_type: 'else_into',
      edge_name: '不满足任何条件',
      condition_rules: [],
      actions: [],
    };
    if (edgeTab.elseInto === 'parseFailedEdge') {
      edge.to_node_id = nodeId;
      edge.actions = [
        this.actionSetParseFailed(true),
      ];
  ...
  setNodeConnections(nodeConnections, nodeId, toNodeId) {
    if (toNodeId === null || toNodeId === '-1' || toNodeId === nodeId || nodeConnections[toNodeId] === undefined) return;
    if (toNodeId === '0') {
  ...
  traverseQQEdge(nodeId, edge, nodeConnections) {
    ...
    edge.candidate_edges.forEach((e) => {
      ...
      if (toNodeId === null || toNodeId === '-1' || toNodeId === nodeId) {
        return;
      }
  ...
  checkNodeFormat(uiNode, nodeConnections, nodeType, nodeId) {
    if (nodeType === 'entry') {
      ...
      if (!uiNode.dialogue2SettingTab.skipDialogue && (!uiNode.dialogue2SettingTab.initialResponse || uiNode.dialogue2SettingTab.initialResponse === '')) {
        uiNode.warnings.push({
  ...
  parseCustomParser(uiNode, customParsers) {
    if (uiNode.nodeType === 'entry') {
      ...
      normalEdges.forEach((edge) => {
        if (edge.condition_rules &&
          ...
          andRules.forEach((rule) => {
            if (rule.functions && rule.functions.length > 0) {
              rule.functions.forEach((func) => {
                if (func.function_name === 'custom_parser' && func.content.parser !== '' && func.content.parser !== undefined) {
      ...
      params.forEach((param) => {
        param.parsers.forEach((parser) => {
          if (parser.funcName === 'custom_parser' && parser.content.parser !== '' && parser.content.parser !== undefined) {
      ...
      normalEdges.forEach((edge) => {
        edge.actions.forEach((action) => {
          if (action.parser === 'custom_parser' && action.customParserType !== '' && action.customParserType !== undefined) {
            customParsers[action.customParserType] = action.function.content.parserData;
          }
        });
      });
    }
  ...
  getExitEdges(nodes) {
    ...
    nodes.forEach((node) => {
      ...
      if (nodeId === '0') return;
      node.edges.forEach((edge) => {
        if (edge.to_node_id === '0') {
          const exitEdge = {};
          exitEdge.node_id = node.node_id;
          exitEdge.node_name = node.description;
          exitEdge.edge_name = edge.edge_name || '';
          edges.push(exitEdge);
        }
      });
    });
