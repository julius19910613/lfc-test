...
const ActionType = {
  ...
  SelfIncrement: 'self_increment',
  WebAPI: 'web_api',
  JSScript: 'js_script',
  ResponseText: 'response_text',
  ResponseVoice: 'response_msg',
  RemoveKey: 'remove_key',
  SubScenario: 'sub_scenario',
};
const Parsers = ['common_parser', 'task_parser', 'hotel_parser', 'user_custom_parser', 'polarity_parser', 'nlu_parser', 'reg_parser', 'custom_parser', 'dialog_act_parser'];
const NLUParserMap = {
  ADDRESS: 'ADDRESS',
  QUANTITY: 'QUANTITY',
...
export default {
  nodeType2Tabs() {
    const dialogue2 = ['dialogue2SettingTab', 'understandSetting2', 'edgeTab2'];
    if (store.getters.voiceSet) {
      dialogue2.push('voiceSettingTab2');
    }
    return {
      entry: ['settingBasicTab', 'triggerTab', 'edgeTab'],
      dialogue: ['settingTab', 'edgeTab'],
      'dialogue_2.0': dialogue2,
      nlu_pc: ['settingBasicTab', 'entityCollectingTab', 'edgeTab'],
      sub_scenario: ['dialogue2SettingTab', 'subScenarioTab', 'edgeTab2'],
  ...
  getNodeTypes(context) {
    const nodeTypes = [
      ...
      {
        type: 'sub_scenario',
        name: context.$t('task_engine_v2.node_type.sub_scenario'),
        image: DialogueIcon,
        description: context.$t('task_engine_v2.node_type.sub_scenario_description'),
      },
      {
        type: 'restful',
  ...
  getActionOptions(context) {
    ...
    {
      text: context.$t('task_engine_v2.condition_action_block.action.self_increament'),
      value: ActionType.SelfIncrement,
    },
    {
    ...
    // {
    //   text: context.$t('task_engine_v2.condition_action_block.action.response_voice'),
    ...
    if (context.currentTab === 'subScenarioTab') {
      options.push({
        text: context.$t('task_engine_v2.condition_action_block.action.sub_scenario'),
        value: ActionType.SubScenario,
      });
    }
    return options;
  },
  ...
  getActionOptionMap(context) {
    ...
    const selfIncreament = ['self_increament'];
    return {
      ...
      [ActionType.SelfIncrement]: selfIncreament.map((self) => {
        const key = `task_engine_v2.condition_action_block.func.${self}`;
        return {
          text: context.$t(key),
          value: self,
        };
