  ...
  methods: {
    ...
    deleteMapping(itemData) {
      ...
      that.$api.deleteMappingList(fileName).then(() => {
        ...
        this.$message({
          message: that.$t('error_msg.delete_success'),
          type: 'success',
          showClose: true,
        });
        that.loadList();
        ...
        this.$message({
          message: `${that.$t('error_msg.delete_fail')}:${err.message}`,
          type: 'error',
          showClose: true,
        });
        that.$emit('endLoading');
    ...
    loadList() {
      ...
      if (userid !== '4c8cad6375894d487327cd1e7c5d5ef4') { // add template data for none template user
        that.$api.getTemplateMappingList().then((data) => {
          ...
          this.$message({
            message: `${that.$t('error_msg.data_format_err')}:${err.message}`,
            type: 'error',
            showClose: true,
          });
          that.$emit('endLoading');
      ...
      that.$api.getMappingList().then((data) => {
        ...
        if (files) {
          ...
          this.$message({
            message: that.$t('error_msg.data_format_err'),
            type: 'error',
            showClose: true,
          });
        }
