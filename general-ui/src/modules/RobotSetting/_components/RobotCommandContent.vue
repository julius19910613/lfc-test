  ...
  <div id="card-content" class="card">
    <div id="card-content-header">
      ...
      <div class="top-button">
        <!-- <template v-if="importErrorID !== undefined">
          <text-button
            button-type="error"
            @click="downErrorReport">
            {{ $t('robot_command.down_error_msg') }}
          </text-button>
        </template> -->
        <!-- <text-button
          button-type="default"
          iconType="data_import"
          @click="importCommand">
          {{ $t('robot_command.data_import') }}
        </text-button>
        <text-button
          button-type="default"
          iconType="export_file"
          @click="exportCommand">
          {{ $t('robot_command.all_export') }}
        </text-button> -->
      </div>
    </div>
    ...
    <div class="upload-box" v-show="upload && uploadLoading">
      <upload-progress :importPercent="uploadPercent" :visible="upload && uploadLoading"></upload-progress>
    </div>
    <div class="upload-box" v-show="upload && !uploadLoading">
      <op-result class="upload-result" v-bind="resultData"
        @primaryOp="handlePrimaryOp" @borderOp="handleResultOperation" @linkOp="handleDLErrorReport"></op-result>
    </div>
  </div>
</template>
<script>
...
import UploadProgress from '@/components/basic/UploadProgress/UploadProgress';
import OpResult from '@/components/basic/OpResult/OpResult';
import CommandEditPop from './RobotCommandEditPop';
import MoveToPop from './RobotCommandMoveToPop';
import api from '../_api/command';
import ImportCommand from './ImportCommandPop';

function getStatusMsg(subtitle = '', status = 0, link = false) {
  const info = {
    status: 'success',
    btnPrimaryText: this.$t('robot_command.command_manage'),
    title: this.$t('error_msg.import_done'),
    subtitle,
  };
  if (status === -1) {
    Object.assign(info, {
      status: 'fail',
      btnPrimaryText: this.$t('robot_command.re_import'),
      btnBorderText: [this.$t('robot_command.command_manage')],
      title: this.$t('error_msg.import_fail'),
      linktext: link ? this.$t('error_msg.down_err_msg') : '',
    });
  }
  return info;
}
...
export default {
  ...
  data() {
    return {
      ...
      curPageIdx: 1,
      pageLimit: 25,
      importErrorID: undefined,
      upload: false,
      uploadLoading: false,
      uploadPercent: 0,
      intervalCode: -1,
      resultData: getStatusMsg.call(this, '', -1),
    };
  },
  computed: {
  ...
  methods: {
      ...
      const options = {
        ...
        callback: {
          ok: (addedCommand) => {
            ...
            if (addedCommand.name.length > 60) {
              this.$notifyFail(this.$t('robot_command.error.add_command_maximun'));
              return;
            }
    ...
    popEditCommand(command) {
      const options = {
        ...
        callback: {
          ok: (editedCommand) => {
            if (editedCommand.name.length > 60) {
              this.$notifyFail(this.$t('robot_command.error.edit_command_maximun'));
              return;
            }
    ...
    importCommand() {
      const that = this;
      const options = {
        component: ImportCommand,
        title: this.$t('robot_command.import_pop.title'),
        validate: true,
        callback: {
          ok: (info) => {
            that.uploadFile(info);
          },
        },
      };
      this.$pop(options);
    ...
    uploadFile(data) {
      ...
      that.setErrorImportID();
      that.$api.commandImport.call(this, data)
        .then((d) => {
          // that.$emit('endLoading');
          if (d.status === 0) {
            that.getLoadStatus(d.result);
          } else {
            that.$emit('endLoading');
            that.$notifyFail(d.message);
            this.uploadLoading = false;
            this.resultData = getStatusMsg.call(this, d.message, -1);
          }
        })
        .catch((err) => {
          this.uploadLoading = false;
          ...
          if (err.response.status < 500) {
            this.resultData = getStatusMsg.call(this, err.response.data.message, -1);
            that.$notifyFail(err.response.data.message);
          } else {
            this.resultData = getStatusMsg.call(this, err.message, -1);
            // that.$notifyFail(err.message);
          }
        });
    },
    getLoadStatus(id) {
      const that = this;
      const pms = new Promise((r, j) => {
        that.intervalImportStatus(id, r, j);
      });
      pms.then(() => {
        console.log('success');
        this.resultData = getStatusMsg.call(this);
      }).catch(() => {
        that.setErrorImportID(id);
        console.log('error');
      }).finally(() => {
        this.uploadLoading = false;
    ...
    intervalImportStatus(id, r, j) {
      this.intervalCode = setInterval(() => {
        this.getCommandImport(id, r, j);
      }, 1000);
    },
    getCommandImport(id, r, j) {
      const that = this;
      that.$api.getCommandImport.call(this, id)
        .then((d) => {
          if (d.status === 0) {
            this.uploadPercent = d.result;
            if (d.result === 100) {
              r();
            }
          } else {
            j();
            this.resultData = getStatusMsg.call(this, d.message, -1, true);
            that.$notifyFail(d.message);
          }
        })
        .catch((err) => {
          j();
          if (err.response.status < 500) {
            this.resultData = getStatusMsg.call(this, err.response.data.message, -1, true);
            that.$notifyFail(err.response.data.message);
          } else {
            this.resultData = getStatusMsg.call(this, err.message, -1, true);
            // that.$notifyFail(err.message);
          }
        });
    },
    setErrorImportID(id) {
      this.importErrorID = id;
    },
    downErrorReport() {
      const that = this;
      that.$api.ImportCommandReport.call(this, this.importErrorID)
        .then((res) => {
          const info = misc.getRemoteFile(res);
          misc.downloadRawFile(info.blob, info.filename);
        })
        .catch((err) => {
          console.log(err);
          that.$notifyFail(that.$t('robot_command.error.export_error'));
        });
    },
    exportCommand() {
      const that = this;
      that.$api.commandExport.call(this)
        .then((res) => {
          const info = misc.getRemoteFile(res);
          misc.downloadRawFile(info.blob, info.filename);
        })
