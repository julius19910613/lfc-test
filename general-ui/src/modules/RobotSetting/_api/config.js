...
const SAVE_DIMENSION_EDIT = '/adm/system/dimension/save';
...
function fetchDimensionList(appId) {
  return this.$reqGet(`${FECTH_DIMENSION_LIST}/${appId}`).then(res => res.data);
}

function saveDimensionEdit(params) {
  return this.$reqPost(`${SAVE_DIMENSION_EDIT}`, params).then(res => res.data);
}
