<template>
  <div class="robot-special-words">
      ...
      <div class="card-content">
        ...
        <div class="content" @wheel="handleScroll" ref="container">
          <div class="config-block" ref="robot-block" data-key="robot">
            ...
            <div class="row" v-if="robotDialogueLang!=='en'">
            ...
            <!-- 多维度 -->
            <div class="row">
              <div class="row-title">{{$t('robot_config.dimension.title')}}</div>
              <ul class="tag-list">
                <li v-for="(item, index) in dimensionList" :key="index">{{item.name}}：<span v-for="(tag, idx) in item.list" :key="idx">{{tag.name}} <template v-if="idx !== item.list.length - 1">、</template></span></li> 
                <icon :size=12 icon-type="edit_blue" @click="showEditTagModal = true" class="dimension-icon"></icon>
              </ul>
            </div>
          </div>
          <template v-for="config in configs">
            <div v-if="config.status" :key="config.key" class="config-block" :data-key="config.key" ref="block">
              <div class="block-title row" :class = "{'is-SSM':config.key === 'ssm'}">{{ $t(`robot_config.${config.key}.title`) }}{{ $t('general.setting') }}</div>
              ...
              <template v-if="config.hasValidChildren" >
                ...
                <template v-if="config.key === 'ssm'">
                  <!-- 近似问添加 -->
                  <div v-for="group in config.children" :key="group.key">
                    <div class="row row-special" v-if="group.type==='group'">
                      <div class="row-title">
                        <div v-if="group.showTips" class="row-tooltip-box"
                        v-tooltip="{'msg': $t(`robot_config.${config.key}.${group.key}-tooltip`)}">
                          <icon iconType="info" enableHover :size="18"></icon>
                        </div>
                        {{ $t(`robot_config.${config.key}.${group.key}-title`) }}
                        <p style="margin-top: 20px;" v-show="group.key==='ssm_underline_config'">
                          <span style="display: inline-block; background-color: #1875f0;
                      ...
                      <template v-for="child in config.children">
                        <div v-if="child.status && child.group === group.key" :key="child.key" class="content-row">
                          <template v-if="!child.alwaysOn">
                          <toggle class="content-row-toggle" v-model="child.on"
                          :disabled="!canEdit || child.key === 'ssm_context_support_online'"
                          @input="handleConfigChange(child, $event)"/>
                          </template>
                          <div class="content-row-desc" >
                            ...
                            <template v-else>
                              ...
                              :max="child.max||1000000" :min="child.min!==undefined?child.min:-1000000"
                              @keyup.enter="finishEdit(child)" @input="checkThreshold(child)"
                              @blur="cancelEditConfig" @keyup.esc="cancelEditConfig">
                              {{ $t(`robot_config.${config.key}.${child.key}-suf`) }}
                              </template>
                              <template v-if="child.type === 'time-range'">
                              {{ $t(`robot_config.${config.key}.${child.key}-pre`) }}
                          ...
                          <template v-if="canEdit && child.type !== 'switch' && editTarget === undefined
                          && !robotNameEdit && (child.on || child.alwaysOn) && child.group !== 'ssm_config_online'">
                ...
                <div class="row" v-if="config.key != 'ssm'">
              ...
              <template v-if="config.files && canEdit">
                <div class="row">
    ...
    <el-dialog
      class="emotibot no-body-padding"
      :title="$t('robot_config.dimension.set_bot_dimension')"
      :visible.sync="showEditTagModal"
      :close-on-click-modal="false"
      width="650px"
      center>
      <edit-dimension @handleCloseModel="handleCloseModel">
      </edit-dimension>
    </el-dialog>
  </div>
</template>
...
import EditDimension from './_components/EditDimension';
import qs from 'qs';
...
export default {
  ...
  computed: {
    ...
    activeBarStyle() {
      let index = 0;
      const that = this;
      const validConfigs = this.configs.filter(item => item.status !== false);
      ...
      configs: [
        {
          ...
          children: [
            ...
            {
              key: 'faq-related-question',
              type: 'group',
              showTips: true,
            },
            {
              key: 'faq-related-question-switch',
              group: 'faq-related-question',
              on: false,
              module: 'controller',
              type: 'switch',
              status: true,
            ...
            {
              key: 'faq-related-question-count',
              group: 'faq-related-question',
              value: 8,
              module: 'controller',
              status: true,
            ...
            {
              key: 'faq-similar-question',
              type: 'group',
              showTips: true,
            },
            {
              key: 'faq-similar-question-switch',
              group: 'faq-similar-question',
              on: false,
              module: 'controller',
              type: 'switch',
              status: true,
            ...
            {
              key: 'faq-similar-question-count',
              group: 'faq-similar-question',
              module: 'controller',
              value: 3,
              status: true,
            ...
            {
              key: 'faq-recommand-question',
              type: 'group',
              showTips: true,
            },
            {
              key: 'faq-recommand-question-switch',
              group: 'faq-recommand-question',
              on: false,
              module: 'controller',  
              status: true,
              type: 'switch',
            ...
            {
              key: 'faq-recommand-question-count',
              group: 'faq-recommand-question',
              module: 'controller',
              value: 3,
              type: 'number',
            ...
            {
              key: 'ssm_underline_config',
              type: 'group',
            },
            {
              key: 'ssm_context_support_underline',
              group: 'ssm_underline_config',
              on: false,
              value: 0,
              module: '',
              type: 'switch',
            ...
            {
              key: 'ssm_config_online',
              type: 'group',
            },
            {
              key: 'ssm_context_support_online',
              group: 'ssm_config_online',
              on: false,
              value: 0,
              module: '',
              type: 'switch',
  ...
  methods: {
    handleCloseModel(type) {
      this.showEditTagModal = false;
    },
    handleInputNumberChange() {
      this.$refs['edit-input'][0].focus();
    },
    handleInputNumberBlur() {
      const that = this;
      window.setTimeout(() => {
        if (!(that.$refs['edit-input'] && that.$refs['edit-input'][0]
        && that.$refs['edit-input'][0].$el === document.activeElement.parentElement.parentElement)) {
          that.cancelEditConfig();
        }
      }, 50);
    },
    handleScroll() {
    ...
    checkThreshold(child) {
      if (child && child.max && child.min !== undefined) {
        this.editNumber = this.editNumber < child.min ?
          child.min : this.editNumber;
        this.editNumber = this.editNumber >  child.max ?
          child.max : this.editNumber;
      } else {
