...
export default {
  ...
  methods: {
    ...
    handleKey(e, robotQA, obj, type) {
      ...
      if (type === 'question') {
        if (obj === undefined) {
          ...
          // 合并所以问题相似问题
          const totolRelated = [];
          for (let i = 0; i < this.filterQAs.length; i += 1) {
            totolRelated.push(this.filterQAs[i].main_question);
            this.filterQAs[i].relate_questions.forEach((item) => {
              totolRelated.push(item.content);
            });
          }
          const duplicateRelated = totolRelated.filter(item => item === that.updateValue);
          if (duplicateRelated.length > 0) {
            this.$notifyWarn(that.$t('robot_setting.duplicate_answer'));
            return;
          }
    ...
    addSimilaryQuestion(robotQA) {
      ...
      const duplicateRelated = totolRelated.filter(item => item === that.newQuestion);
      if (duplicateRelated.length > 0) {
        this.$notifyWarn(that.$t('robot_setting.duplicate_answer'));
        return;
      }
