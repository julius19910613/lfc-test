  ...
  <div class="chat-tools">
    ...
    <div class="chat-search">
      <div class='chat-search-item'>
        <div class="left">
          APPID&nbsp;&nbsp;<input type="text" v-model.trim="appId" disabled="true">
    ...
    <div class="test-result-table">
      ...
      <div class="result-table-body">
        <div class="table-item">
          <div class="result-box result-box-body" v-for="(val, index) in sheetGroup" :key="`${curPageIdx}${index}`">
            ...
            <span class="response-module" :class="{'error': val.result === false}">
              {{val.actualModule}}
              <div class="icon-container" v-tooltip="val.tips" v-show="val.result !== null && val.tips.length > 0">
...
export default {
  ...
  data() {
    return {
      ...
      sheetGroup: [],
      allData: [],
      scenarioName: this.$t('chat_tools.scenario'),
      correct: undefined,
      keywords: '',
      curPageIdx: 1,
      pageLimit: 50,
  ...
  watch: {
    groupFilter: {
      handler() {
        ...
        const passTotal = this.backupData.filter(item => item.result === true);
        this.correct = ((passTotal.length / this.backupData.length) * 100).toFixed(2);
      },
      deep: true,
    },
  },
  methods: {
      ...
      that.$api.uploadFile(this.file, this.appId)
        .then((res) => {
          if (res.data.status === 0) {
            that.$message({
              showClose: true,
              message: this.$t('chat_tools.import_success'),
              type: 'success',
            });
    ...
    handleTest() {
      ...
      const that = this;
      that.$startPageLoading(this.$t('chat_tools.test_running'));
    ...
    downResult() {
      if (this.sheetGroup.length === 0) {
        this.$notifyFail(this.$t('chat_tools.no_data_to_down'));
        return;
      }
      const filterData = this.sheetGroup.filter(item => item.actualModule === null);
    ...
    handlePageSizeChange(pageSize) {
      ...
      this.curPageIdx = 1;
      this.handlePageChange(this.curPageIdx);
      // this.pageLimit = pageSize;
      // this.toFirstPage();
      // this.sheetGroup = [];
    },
