    ...
    <el-dialog
      ...
      <el-form  :rules="rules"  ref="addNewCls" :model="addNewClsForm" label-width="100px" >
         <el-form-item label="数据名称" prop="name" >
          <el-input v-model="addNewClsForm.name" placeholder="输入名称" size="small" style="width: 75%"></el-input>
        </el-form-item>
         <el-form-item label="关键词" prop="keyword">
          <el-input v-model="addNewClsForm.keyword" placeholder="输入关键词" size="small" style="width: 75%"></el-input>
        </el-form-item>
         <el-form-item label="数据位置" prop="position">
          <el-input v-model="addNewClsForm.position" placeholder="输入数据位置" size="small" style="width: 75%"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    bulkDelete() {
      ...
      this.$api.DataBulkDelete(idList).then((res) => {
        if (res.status === 200) {
          this.$message({
            type: 'success',
            message: '删除成功!',
    ...
    handleEdit(index, row) {
      ...
      this.addNewClsForm.name = row.title;
      this.addNewClsForm.keyword = row.keyword;
      this.addNewClsForm.position = row.position;
      this.addNewClsForm.itemId = row.itemId;
      this.addNewClsForm.summary = row.summary;
      this.addNewClsForm.createTime = row.createTime;
    },
    // 删除
    handleDelete(index, row) {
      ...
      this.$confirm('此操作将删除该条数据, 是否继续?', '提示', {
        ...
        this.$api.deleteOneData(id).then((res) => {
          if (res.status === 200) {
            this.$message({
              type: 'success',
              message: '删除成功!',
    ...
    editclass() {
      ...
      addItem.title = this.addNewClsForm.name;
      addItem.keyword = this.addNewClsForm.keyword;
      addItem.position = this.addNewClsForm.position;
      addItem.itemId = this.addNewClsForm.itemId;
      addItem.summary = this.addNewClsForm.summary;
      addItem.createTime = this.addNewClsForm.createTime;
      this.$api.editDataSortItem(addItem, addItem.id).then((res) => {
        this.$refs.addNewCls.resetFields();
        if (res.status === 200) {
          this.$message({
            type: 'success',
    ...
    addClass() {
      ...
      addItem.position = this.addNewClsForm.position;
      // 添加接口
      this.$api.addDataSortItem(addItem).then((res) => {
        if (res.status === 200) {
          this.$message({
            type: 'success',
            message: '操作成功!',
    ...
    // 获取 tableData
    getAllTableData() {
      this.$api.getdataSortingList(this.currentPage4 - 1, this.pagesize).then((res) => {
        this.currentPage4 = res.pageNumber + 1;
        this.totalData = res.totalElements;
        this.tableData = res.content;
        this.tableData.forEach((el) => {
