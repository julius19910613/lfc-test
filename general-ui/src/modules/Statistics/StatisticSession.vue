      ...
      <div class="page-header">
            <div class="title">
                <div class="title-text">
                    {{ $t('pages.statistics.statistic_session') }}
                </div>
            </div>
            <!-- <text-button @click="doExport" v-if="recordNum > 0 && canExport">{{ $t('general.export') }}</text-button>
            <text-button button-type="disable" v-else>{{ $t('general.export') }}</text-button> -->
            <el-button v-if="recordNum > 0 && canExport"
                size="mini"
      ...
      >
        ...
        >
            <template slot-scope="scope">
                ...
                :content="$t('general.view')">
                <div class="operation-icon ">
                    <click-icon
                        @click="showSessionDetail(scope.$index, scope.row)" 
                        iconType="tableView"
                        >
                    </click-icon>
                </div>
                </el-tooltip>
                <!-- <div @click="showSessionDetail(scope.$index, scope.row)">
                    <img src="../../assets/images/table_view.svg">
                </div> -->
                <!-- <icon icon-type="close" :size="8" ></icon> -->
            </template>
        </el-table-column>
      </el-table>
      <div class="paginator">
        <el-pagination
            ...
            :page-sizes="constant.tableLimitOption"
            :page-size="nowLimit"
...
export default {
  ...
  api: [api, auditAPI, tagAPI, feedbackAPI, staAPI],
  components: {
    FilterField,
    RangePicker,
    DropdownSelect,
  ...
  methods: {
    ...
    searchForMore() {
      ...
      this.$api.getSessionList(that.tableMaxRecord, 1, filter).then(() => {
    ...
    doExport() {
      ...
      if (this.exportStatus) {
        this.$notify({ text: this.$t('statistics.export_running') });
        return;
      }
      ...
      .then((exportID) => {
        ...
        const checkStatus = () => {
          that.$api.getExportStatusNew(exportID).then((data) => {
    ...
    showSessionDetail(index, session) {
      const that = this;
