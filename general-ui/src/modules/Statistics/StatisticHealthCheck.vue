  ...
  <div>
    <div class="health-check-wrapper card h-fill w-fill">
      <div class="loading-wrapper checking-wrapper" v-show="isChecking">
        <el-progress type="circle" :percentage="Number(reportProgress)"></el-progress>
...
export default {
  ...
  methods: {
    ...
    handleGenerateReport() {
      ...
      if (this.healthData && !this.healthData.recheck && !this.generateReportError) {
        return;
      }
    ...
    getReportStatus() {
      this.$api.getHealthReportStatus(this.robotID).then((res) => {
        ...
        if (res.result.status === 'progress') {
          ...
          this.$endPageLoading();
        } else if (res.result.status === 'nodata') {
          this.isChecking = false;
          this.$endPageLoading();
        } else {
