<template>
  <div id="pop-wordbank-edit">
    ...
    <!-- <div id="edit-footer"> -->
      <!-- <v-pagination class="emotibot" size="small" :total="curTotal" :pageIndex="curPageIdx" :pageSize="pageLimit" :layout="['prev', 'pager', 'next', 'jumper']" @page-change="handlePageChange"></v-pagination> -->
    <!-- </div> -->
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      synonymTooltip: {
        msg: this.$t('knowledge_graph.error.synonym_duplicate'),
        eventOnly: true,
        errorType: true,
        alignLeft: true,
      },
  ...
  watch: {
    ...
    // isDuplicate() {
    //   if (!this.isDuplicate) {
    //     const event = new Event('tooltips-hide');
  ...
  methods: {
    ...
    addSynonym() {
      ...
      if (this.isDuplicate) {
        const event = new Event('tooltips-show');
        this.$refs.synonymInput.dispatchEvent(event);
        return;
      }
    ...
    updateNameTooltip() {
      if (!this.isWordbankNameValid) {
        ...
        if (this.isWordbankNameEmpty) {
          this.wordbankNameTooltip.msg = this.$t('knowledge_graph.error.name_empty');
        } else if (this.isWordbankNameDuplicate) {
          this.wordbankNameTooltip.msg = this.$t('knowledge_graph.error.name_duplicate');
        } // TODO: else if wordbank name too long
        const reload = new Event('tooltips-reload');
