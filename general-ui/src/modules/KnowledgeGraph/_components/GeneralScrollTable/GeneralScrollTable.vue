  ...
  <div class="general-table-container" :class="[showEmpty ? 'show-empty': '']">
            ...
            <div class="tr">
              ...
              <template v-for="headerLeft in tableColumnLeft()">
                <div
                  ...
                  :title="headerLeft.text"
                >
                  {{ getEllipsisStr(headerLeft.text, 11)  || '-'}}
                  <div
                    v-if="headerLeft.info && headerLeft.info !== ''"
          ...
          <div ref="tableHeaderScroll" class="table-layout-middle">
            <div class="tr">
              <template v-for="header in tableHeader">
                <div
                  v-if="isTableColumnMiddle(header)"
                  :key="header.key"
                  ...
                  :title="header.text"
                >
                  {{ getEllipsisStr(header.text, 11)  || '-'}}
                  <div
                    v-if="header.info && header.info !== ''"
                    :ref="`${header.key}-info`"
                    class="table-header-icon"
                    v-tooltip="headerInfoTooltip"
            ...
            <div class="tr">
              <template v-for="headerRight in tableColumnRight()">
                <div
                  ...
                  class="td table-header-item"
                  :title="headerRight.text"
                >
                  {{ getEllipsisStr(headerRight.text, 11)  || '-'}}
                  <div
                    v-if="headerRight.info && headerRight.info !== ''"
              ...
              <div
                v-if="hasAction"
                class="td table-col-action table-header-item"
                :class="{'multi-action': action.length > 1}"
                :style="multiActionStyle(action.length)"
              >{{ $t('general.actions') }}</div>
    ...
    <div v-else-if="showEmpty && tableData.length <= 0" class="empty-display">
      <template v-if="showEmptyMsg">
        <div class="empty-msg" v-for="msg in showEmptyMsg" :key="msg">{{ msg }}</div>
      </template>
      <template v-else-if="$slots.emptyMsg">
        <slot name="emptyMsg"></slot>
      </template>
      <template v-else>{{ $t('general.no_data') }}</template>
    </div>
    <div v-else class="general-table-body">
      <div
        class="table general-table"
        :class="[autoHeight ? 'auto-height' : '', fontClass]"
      ...
      >
        <div class="tbody" :class="[onclickRow ? 'clickable-row' : '']">
          <div
            ...
            :class="{'has-scroll-x': isScrollbarXShown}"
            @wheel="scrollRight"
          >
            <div
              v-for="(data, idx) in tableData"
              :key="idx"
              :class="{'highlight': data.highlight, 'row-hover': data.isHover}"
              class="tr"
            ...
            >
              ...
              <template v-for="headerLeft in tableColumnLeft()">
                <div
                  :key="uniqueId(headerLeft.key, idx)"
                  :style="{width: headerLeft.width}"
                  :class="columnClassObject(headerLeft)"
                ...
                >
                  <div v-if="headerLeft.type === 'tag'">
                    <tag
                      class="tags"
                      v-for="(tag, tagIdx) in data[headerLeft.key]"
                      :key="`${tagIdx}-${tag}`"
                      :fontClass="fontClass"
                      :title="tag"
                      >{{ getEllipsisStr(tag) }}</tag>
                  </div>
                  ...
                  <template v-else-if="headerLeft.type === 'toggle'">
                    <toggle
                      ...
                      @change="data[headerLeft.key].onclick(data, idx)"
                    ></toggle>
                  </template>
                  <template v-else-if="headerLeft.type === 'tooltip'">
                    <span
                      class="actions"
                      v-for="act in data[headerLeft.key]"
                      :key="act.text"
                      :class="act.type"
                      @click="act.onclick(data, headerLeft.key, idx)"
                    >{{act.text || '-'}}</span>
                  </template>
                  ...
                  <template v-else-if="headerLeft.type === 'icon'">
                    <icon
                      v-if="data[headerLeft.key]"
                      :icon-type="data[headerLeft.key].iconType"
                      :size="data[headerLeft.key].iconSize"
                    ></icon>
                  </template>
                  ...
                  <div
                    ...
                    :title="data[headerLeft.key]"
                  >{{ data[headerLeft.key] }}</div>
                </div>
              </template>
            </div>
          </div>
          <!-- ./end of tablebody table-layout-left -->
              ...
              <template v-for="header in tableHeader">
                ...
                >
                  <div v-if="header.type === 'tag'">
                    <tag
                      class="tags"
                      v-for="(tag, tagIdx) in data[header.key]"
                      :key="`${tagIdx}-${tag}`"
                      :fontClass="fontClass"
                      :title="tag"
                    >{{ getEllipsisStr(tag) }}</tag>
                  ...
                  <template v-else-if="header.type === 'toggle'">
                    <toggle
                      ...
                      @change="data[header.key].onclick(data, idx)"
                    ></toggle>
                  </template>
                  <template v-else-if="header.type === 'tooltip'">
                      <div
                        v-for="(act, index) in data[header.key]"
                        style="position: relative"
                        :key="index"
                      >
                      <!-- <PopoverComp :extDatas="act.options" :tagsData="act.text"/> -->
                      <!-- {{act.options.canEdit}} {{`${act.options.extData.category}` === "2"?"html":""}}-{{act.options.extData.category}} -->
                      <template v-if="act.options.canEdit">
                        <PopoverComp :extDatas="act.options" :tagsData="act.text" :subtype='`${act.options.extData.category}` === "2"?"html":""'/>
                      </template>
                      <template v-else>
                        <tag class="tags" v-if="typeof act.text ==='string'">{{ act.text  || '-' }}</tag>
                        <tag
                          class="tags"
                          v-else
                          v-for="(tag, tagIdx) in act.text"
                          :key="`${tagIdx}-${tag}`"
                        >{{ getEllipsisStr(tag, 8) || '-'}}</tag>
                      </template>
                    </div>
                  </template>
                  <template v-else-if="header.type === 'tooltip2'">
                    <div
                      v-for="(act, index) in data[header.key]"
                      style="position: relative"
                      :key="index"
                    >
                      <div
                        ...
                        :key="act.text.join()"
                        :class="act.type"
                        @click.stop="act.onclick($event, data, header.key, idx)"
                      >
                        <tag
                          class="tags"
                          v-for="(tag, tagIdx) in act.text"
                      ...
                      <table-column-tooltip
                        v-if="act.showColumnTooltip"
                        :showColumnTooltip="act.showColumnTooltip"
                        :option="act.options"
                        :tableHeader="tableHeader"
                        :selectedData="data"
                  ...
                  <template v-else-if="header.type === 'icon'">
                    <icon
                      v-if="data[header.key]"
                      :icon-type="data[header.key].iconType"
                      :size="data[header.key].iconSize"
                    ></icon>
                  </template>
          ...
          <!-- ./end of tablebody table-layout-middle -->
          ...
          >
            ...
            >
              <template v-for="headerRight in tableColumnRight()">
                <div
                  :key="uniqueId(headerRight.key, idx)"
                  :style="{width: headerRight.width}"
                  :class="columnClassObject(headerRight)"
                  class="td table-body-item"
                  @click="handleOnclickRow(onclickRow, data, idx)"
                >
                  <div v-if="headerRight.type === 'tag'">
                    <tag
                      class="tags"
                      v-for="(tag, tagIdx) in data[headerRight.key]"
                      :key="`${tagIdx}-${tag}`"
                      :fontClass="fontClass"
                      :title="tag"
                    >{{ getEllipsisStr(tag) }}</tag>
                  ...
                  <template v-else-if="headerRight.type === 'toggle'">
                    <toggle
                      ...
                      @change="data[headerRight.key].onclick(data, idx)"
                    ></toggle>
                  </template>
                  <template v-else-if="headerRight.type === 'tooltip'">
                    <span
                      class="actions"
                      v-for="act in data[headerRight.key]"
                      :key="act.text"
                      :class="act.type"
                      @click.stop="act.onclick(data, headerRight.key, idx)"
                    >{{act.text || '-'}}</span>
                  </template>
                  ...
                  <template v-else-if="headerRight.type === 'icon'">
                    <icon
                      v-if="data[headerRight.key]"
                      :icon-type="data[headerRight.key].iconType"
                      :size="data[headerRight.key].iconSize"
                    ></icon>
                  </template>
                  ...
                  >{{ data[headerRight.key] }}</div>
                </div>
              </template>

              <div
                v-if="hasAction"
                class="td table-col-action"
                :class="{'multi-action': action.length > 1}"
                :style="multiActionStyle(action.length)"
              >
                <template v-for="act in action">
                  <span
                    class="actions"
                    :key="act.text"
                    :class="[act.type, {'action-disabled':(data.action_clickable && !data.action_clickable[act.key]) }]"
                    @click="act.onclick(data, idx)"
              ...
              <template v-if="$scopedSlots.menu">
                ...
                <div v-if="idx === indexOfShowMenu" class="td menu-container">
                  <slot name="menu" :rowData="data" :rowIndex="idx"></slot>
                </div>
              </template>
              <div v-if="allowCustomHeader" class="td table-col-set"></div>
            </div>
          </div>
          <!-- ./end of tablebody table-layout-right -->
        </div>
      </div>
    </div>
  </div>
...
import PopoverComp from '../tooltips/PopoverComp';

/** Max-width of tooltips is 300px
 *  Let tooltips alignLeft if column is too close to     righthand side of table */
...
export default {
  ...
  methods: {
    ...
    updateHeaderInfoTooltip(header) {
      ...
      /** Max-width of tooltips is 300px,
      /*  Let tooltips alignLeft if infoIcon is too close to righthand side of table */
      ...
      infoIconBlockDom.dispatchEvent(event.createEvent('tooltips-reload'));
    },
    copyText(e, header) {
    ...
    showFullText(e, text, refName) {
      ...
      ref.dispatchEvent(event.createEvent('tooltips-reload'));
      ref.dispatchEvent(event.createEvent('tooltips-show'));
    },
    hideFullText(e, refName) {
      const that = this;
      if (!misc.isEllipsisActive(e.target)) return;
      const ref = Array.isArray(that.$refs[refName]) ? that.$refs[refName][0] : that.$refs[refName];
      ref.dispatchEvent(event.createEvent('tooltips-hide'));
    },
    moreClick(index, e) {
    ...
    closeColumnTooltip(result) {
      // console.log(result);
      // const data = result[0];
      // const key = result[1];
      // const selectedEntity = this.tableData.filter(d => d.id === data.id)[0];
      // const entityIdx = this.tableData.indexOf(selectedEntity);
      // selectedEntity[key][0].showColumnTooltip = !selectedEntity[key][0].showColumnTooltip;
      // this.tableData.splice(entityIdx, 1, selectedEntity);
      this.$emit('closeTooltip', result);
    },
    setSelectedTableHeader(selectedHeader) {
      this.selectedTableHeader = selectedHeader;
      localStorage.setItem(`${this.robotID}-savedSelectedHeader`, JSON.stringify(selectedHeader));
    },
    ...
    resetSelectedHeader() {
      const savedSelectedHeader = localStorage.getItem(`${this.robotID}-savedSelectedHeader`);
      if (!savedSelectedHeader) {
        this.useDefaultHeaderSetting = true;
        if (this.allowCustomHeader) {
        ...
        this.selectedTableHeader = JSON.parse(savedSelectedHeader);
        this.useDefaultHeaderSetting = false;
      }
    },
