...
export default {
  ...
  computed: {
    ...
    canExport() {
      const ifHasRight = this.$hasRight('export');
      if (ifHasRight) {
        this.recordTableHeader.forEach((item) => {
          if (item.key === 'test_record' || item.key === 'intent_model') {
            item.type = 'custom';
          }
        });
      } else {
  ...
  methods: {
    ...
    renderRecordData(records, type) {
      return records.map((record) => {
        ...
        let testRecord = text;
        let intentModel = general.timestampToDatetimeString(record.ie_model.updated_time);
        if (this.canExport) {
          testRecord = this.composeRecordDownloadLink(record);
          intentModel = this.composeModelDownloadLink(record);
        }
        const rtn = {
        ...
        callback: {
          ok: () => {
            ...
            that.$api.restoreTestRecord(record.intent_test.id).then(() => {
              ...
              that.$notifyFail(this.$t('intent_engine.restore_fail'));
            }).finally(() => {
