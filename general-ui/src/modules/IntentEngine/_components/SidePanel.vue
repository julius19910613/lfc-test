...
<div id="side-panel">
  ...
  <template v-if="mode==='trainPage'">
    <text-button class="button"
     :height="'40px'"
     :button-type="canEditIntent ? 'primary' : 'disable'"
     @click="checkTestCase">
  ...
  <div class="link" v-if="canEditIntent" @click="toPage('test/records')" style="margin-top: 10px;">
...
export default {
  ...
  computed: {
    ...
    canTrain() {
      return this.shouldTrain && this.hasTrainIntents && this.canEditIntent;
    },
    shouldTrain() {
    ...
    trainStatusChanged(newStatus) {
      ...
      if (prevStatus === undefined) {
        ...
        if (newStatus === TRAIN_STATUS.TRAINED) {
          ...
          this.$notifyFail(this.$t('intent_engine.training_fail'));
        }
        if (newStatus !== TRAIN_STATUS.TRAINING) {
          this.setIsTraining(false);
        }
