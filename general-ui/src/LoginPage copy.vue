  ...
  methods: {
    ...
    loadLogo() {
      ...
      that.$api.getIcon('login').then(
        () => {
          that.$refs.logo.style.backgroundImage = `url("${that.$api.getIconURL(
            'login',
          )}")`;
        },
        () => {
...
.login-page {
  font-family: PingFangHK-Regular, PingFangSC, "Microsoft YaHei",
    "Microsoft JhengHei";
  display: flex;
